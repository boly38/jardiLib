...
class JardiTerm {
  ...
  showMonth() {
      ...
      jt.JardiDocs.listDocuments(options)
      .then((docs) => {
  ...
  moderate() {
      var jt = this;
      let options = {};
      jt.JardiDocs.listContribs(options)
      .then((docs) => {
         jt.content = (Array.isArray(docs) && docs.length > 1) ?
      ...
      .then((contribs) => {
        ...
        term.inputField({autoComplete: allContribsNames , autoCompleteMenu: false},
            (err, cNomSc) => {
                ...
                var contribToExamine = contribs.filter(c => c.doc.nom_scientifique == cNomSc)[0];
                if (contribToExamine && jt.JardiDocs.hasOwnerRole()) {
                  jt._showJsonEntity(contribToExamine);
                  jt.examineDecide(cNomSc, contribToExamine._id);
                  return;
                }
                jt.showDashboard();
            });
  ...
  del() {
    var jt = this;
    jt.JardiDocs.deleteDocuments({"nom":/(tomate|cosmos)/})
    .catch((err) => {
      jt.onError(err);
    })
    .then((deletedCount) => {
