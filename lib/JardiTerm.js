...
class JardiTerm {
  constructor() {
     ...
     jt.content = "";
     jt.history = ["lister"];
     jt.commands = ["contribuer","lister","filtre","supprimer","moderer","examiner","quitter"];
     jt.commandsLabels = ["^gc^:ontribuer","^gl^:ister","^gf^:iltre","^gs^:upprimer","^gm^:oderer","^ge^:xaminer","^gq^:uitter"];
     jt.showWelcome();
  ...
  async examiner() {
      ...
      jt.JardiDocs.listContribs({}, (err, contribs) => {
        ...
        let allContribsNames = contribs.map(c => c.doc.nom);
        if (allContribsNames.length == 0) {
                jt.content = "il n'y a aucune contribution";
                jt.showDashboard();
                return;
        }
        term('Entrez le nom d\'une contribution à examiner: ').eraseLineAfter();
        term.inputField({autoComplete: allContribsNames , autoCompleteMenu: false},
            (err, cNom) => {
                ...
                jt.content = contribs.filter(c => c.doc.nom == cNom)[0];
                jt.showDashboard();
            });
      });
  }
