...
class JardiDocs {
  ...
  initAdmin(cb) {
    ...
    this._canWriteDoc((err, result) => {
      this.isMaintainer = (err !== undefined);
      cb();
    });
  }
  ...
  deleteAllDocuments(cb) {
      this._assumeAvailable();
      this._assumeOwnerRole();
      this.JardiDoc.deleteMany({}, (err, data) => {
          if (err) {
            cb(err);
            return;
          }
  ...
  deleteAllContribs(cb) {
      this._assumeAvailable();
      this._assumeAdminAvailable();
      this.JardiContrib.deleteMany({}, (err, data) => {
          if (err) {
            cb(err);
            return;
          }
    ...
    jf.init((err, files) => {
      ...
      jf.get({}, (err, fileEntries) => {
        ...
        async.series(upserts, function(err, upsertsResults) {
          cb(err, upsertsResults ? upsertsResults.map(u => u.n).reduce((acc,val)=>{return acc+val;}):undefined)
        });
      });
    });
  }
  ...
  //~ private
  _canWriteDoc(cb) {
    this.JardiDoc.updateOne({ nom: 'unknown' }, { nom: 'unknown' }, cb)
  }
